...
export async function POST(request) {
  ...
  const { student_grade, student_class, student_number, password } = body || {};

  if (!student_grade || !student_class || !student_number || !password) {
    return NextResponse.json({ error: '학년, 반, 번호, 비밀번호를 모두 입력해주세요.' }, { status: 400 });
  }
  ...
  if (!Number.isInteger(studentGrade) || !Number.isInteger(studentClass) || !Number.isInteger(studentNumber)) {
    return NextResponse.json({ error: '학년/반/번호를 다시 확인해주세요.' }, { status: 400 });
  }
  ...
  if (!data || data.length === 0) {
    return NextResponse.json({ error: '해당 학년/반/번호로 신청한 내역이 없습니다.' }, { status: 404 });
  }
