...
export async function POST(request) {
  ...
  const { id, student_grade, student_class, student_number, password } = body || {};

  if (!id || !student_grade || !student_class || !student_number || !password) {
  ...
  // 취소하려는 행이 정말 이 학년/반/번호의 신청이 맞는지 먼저 확인
  const { data: row, error: fetchError } = await admin
    .from('applications')
    .select('id, student_grade, student_class, student_number, password_hash')
    .eq('id', id)
    .maybeSingle();
  ...
  if (
    row.student_grade !== studentGrade ||
    row.student_class !== studentClass ||
